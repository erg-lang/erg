...
Dictに順番は関係ありません。また、重複する要素を持つことも出来ません。この点でDictはSetと似ています。
Dictはキー付きのSetと言うこともできるでしょう。
...
キー・値の型は単一でなくてもよい。そのような辞書を __非等質な辞書(heterogenous dict)__ という。
...
しかし、違う型のキーに同じ型の値、または同じ型のキーに違う型の値をあてることはできない。
このような場合は代わりにOr型を使う。
