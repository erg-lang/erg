...
```erg
# current_level = 1
f(x, y) (: gen ?V<2>, gen ?W<2> -> gen ?L<2>.AddO) =
    id(x) + x
```

型制約に現在より低いレベルの型変数が混じっている場合は、具体化しません(ここでは`?V`がそれになります)。
具体化する前も、型制約内の型変数から先に具体化します。
型制約が相互再帰している場合、どちらか片方を単なる型変数(`'T: Type`)とします。

```erg
id: ?T<1> -> ?U<1>
f(x, y) (: |'W: Type, 'R: > 'W| (?V<2>(<: ?T<1>), 'W) -> gen ?L<2>.AddO) =
    id(x) + x
```

```erg
f(x, y) (: |'W: Type, 'R :> 'W, ?U<1> <: 'L <: Add('R)| (?V<2>(<: ?T<1>), 'W) -> 'L.AddO) =
    id(x) + x
```
...
```erg
# ?U<2> --> ?T<2>
f(x, y) (: |'W: Type, 'R :> 'W, ?T<2> <: 'L <: Add('R)| (?V<2>(<: ?T<2>), 'W) -> 'L.AddO) =
    id(x) + x
# current_level = 1
...
それに依存する型変数も同じくType型変数になります(`'V: Type`)。

```erg
f(x, y) (: |'W: Type, 'R :> 'W, 'V: Type, 'V <: 'L <: Add('R)| ('V, 'W) -> 'L.AddO) =
    id(x) + x
...
f(10, 1) (: subst_call_ret([inst {10}, inst {1}], inst |'W: Type, 'R :> 'W, 'V: Type, 'V <: 'L <: Add('R)| ('V, 'W) -> 'L.AddO)
```

```erg
f(10, 1) (: subst_call_ret([inst {10}, inst {1}], (?V<1>, ?W<1>) -> ?L<1>(:> ?W<1>, <: Add(?R(:> ?W<1>)<1>).AddO))
```
...
# ?W<1>(:> {10})
# ?R<1>(:> {1})
# ?W(:> {10})<1> <: ?L<1> <: Add(?R<1>(:> ?W(:> {1}))) <: Add(Nat) == Nat
# 直列化すると
# {10} <: ?W<1> <: ?L<1> <: Add({1}) <: Add(?W) <: Add(?R)
# Add(?R)(:> {10})の最小実装トレイトはAdd(Nat) == Natで、Addは第一引数に関して共変なので
# {10} <: ?W<1>(<: Nat) <: ?L<1> <: Add(?W<1>(<: Nat)) <: Add(Nat) == Nat
# ?L(:> {10}, <: Nat).AddO == Nat # 候補が一つしかない場合は評価を確定する
f(10, 1) (: (?W(:> {10}), ?R(:> {1})) -> Nat)
...
```erg
f|W: Type, R :> W, V: Type, V <: L <: Add(R)|(x: V, y: W): L.AddO = id(x) + y
id|T: Type|(x: T): T = x
