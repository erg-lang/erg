...
```erg
id: ?T<1> -> ?U<1>
f(x, y) (: |'W: Type| (?V(<: ?T<1>)<2>, 'W) -> gen ?L<2>.AddO) =
    id(x) + x
```

```erg
f(x, y) (: |'W: Type, ?U<1> <: 'L <: Add('W)| (?V(<: ?T<1>)<2>, 'W) -> 'L.AddO) =
    id(x) + x
```
...
```erg
# ?U<2> --> ?T<2>
f(x, y) (: |'W: Type, ?T<2> <: 'L <: Add('W)| (?V(<: ?T<2>)<2>, 'W) -> 'L.AddO) =
    id(x) + x
# current_level = 1
...
```erg
f(x, y) (: |'W: Type, 'V: Type, 'V <: 'L <: Add('W)| ('V, 'W) -> 'L.AddO) =
    id(x) + x
id(x) (: |'T: Type| 'T -> gen 'T) = x
```

`'X <: 'Y <: Z`(ただし`'Y`は他の場所で定義されていない)となっている場合、`'Y`を`'X`で単一化できます。

```erg
f x, y (: |'W: Type, 'V: Type, 'V <: Add('W)| ('V, 'W) -> 'V.AddO) =
    id(x) + y
id(x) (: 'T -> 'T) = x

f(10, 1) (: subst_call_ret([inst {10}, inst {1}], inst |'W: Type, 'V <: Add('W)| ('V, 'W) -> 'V.AddO)
```

```erg
f(10, 1) (: subst_call_ret([inst {10}, inst {1}], (?V<1>(<: Add(?W<1>)), ?W<1>) -> ?V<1>.AddO))
```
...
# ?V(:> {10} <: Add(?W<1>))<1>
# ?W(:> {1})<1>
# ?W(:> {1})<1> <: ?V<1> (:> {10}, <: Add(?W(:> {1})<1>))
# 直列化すると
# {1} <: ?W<1> or {10} <: ?V<1> <: Add({1}) <: Add(?W<1>)
# Add(?W)(:> ?V)の最小実装トレイトはAdd(Nat) == Natで、Addは第一引数に関して共変なので
# {10} <: ?W<1> or {1} <: ?V<1> <: Add(?W<1>) <: Add(Nat) == Nat
# ?V(:> ?W(:> {10}) or {1}, <: Nat).AddO == Nat # 候補が一つしかない場合は評価を確定する
...
```erg
f|W: Type, V <: Add(W)|(x: V, y: W): V.AddO = id(x) + y
id|T: Type|(x: T): T = x
