...
Erg 可以将参数(类型参数)传递给类型定义。带有类型参数的 `Option`、`List` 等称为多项式类型。这些本身不是类型，但它们通过应用参数成为类型。诸如 `Int`、`Str` 等没有参数的类型称为简单类型(标量类型)
...
像 `List T` 这样的类型可以看作是 `Type -> Type` 类型的函数，它以 `T` 类型为参数并返回 `List T` 类型(在类型论中也称为 Kind)。像 `List T` 这样的类型专门称为多态类型，而 `List` 本身称为一元 Kind
...
Erg 的数组(List)就是 Python 所说的列表。`[诠释; 3]`是一个数组类，包含三个`Int`类型的对象
...
类型参数也可以用于类型对象以外的类型。例如数组类型`[Int; 3]` 是 `List Int, 3` 的语法糖。如果类型实现重叠，用户必须明确选择一个
