...
Algebraic types are types that are generated by operating on the type as if it were an algebra.
Operations handled by them include Union, Intersection, Diff, and Complement.
Normal classes can only perform Union, and other operations will result in a type error.

## Union

The Union type can give multiple possibilities for the type. As the name suggests, they are generated by the `or` operator.
...
The Diff type is obtained by the `not` operation.
It is better to use `and not` as a closer notation to the English text, but it is recommended to use just `not` because it fits better alongside `and` and `or`.
...
Complement is obtained by the `not` operation, which is a unary operation. The `not T` type is a shorthand for `{=} not T`.
...
There are two types of algebraic types: apparent algebraic types that can be simplified and "true algebraic types" that cannot be further simplified.
The "apparent algebraic types" include Enum, Interval, and the record types `or` and `and`.
To remove the warning, you need to simplify or define the type.
