...
impl<'a> HIRVisitor<'a> {
    ...
    fn visit_expr_t(&self, expr: &Expr, token: &Token) -> Option<Type> {
        ...
        match expr {
            ...
            Expr::Def(def) => self.visit_def_t(def, token),
    ...
    fn visit_def_t(&self, def: &Def, token: &Token) -> Option<Type> {
        self.return_expr_t_if_same(def.sig.ident(), def.sig.ident().name.token(), token)
            .or_else(|| self.visit_block_t(&def.body.block, token))
    }
...
impl<'a> HIRVisitor<'a> {
    /// Returns the smallest expression containing `token`. Literals, accessors, containers, etc. are returned.
    pub(crate) fn visit_hir_info(&self, token: &Token) -> Option<VarInfo> {
        for chunk in self.hir.module.iter() {
            if let Some(expr) = self.visit_expr_info(chunk, token) {
                return Some(expr);
            }
        }
        None
    ...
    fn visit_expr_info(&self, expr: &Expr, token: &Token) -> Option<VarInfo> {
        if expr.ln_end() < token.ln_begin() || expr.ln_begin() > token.ln_end() {
            return None;
        }
        match expr {
            Expr::Lit(_lit) => None,
            Expr::Accessor(acc) => self.visit_acc_info(acc, token),
            Expr::BinOp(bin) => self.visit_bin_info(bin, token),
            Expr::UnaryOp(unary) => self.visit_expr_info(&unary.expr, token),
            Expr::Call(call) => self.visit_call_info(call, token),
            Expr::ClassDef(class_def) => self.visit_class_def_info(class_def, token),
            Expr::Def(def) => self.visit_def_info(def, token),
            Expr::PatchDef(patch_def) => self.visit_patchdef_info(patch_def, token),
            Expr::Lambda(lambda) => self.visit_lambda_info(lambda, token),
            Expr::Array(arr) => self.visit_array_info(arr, token),
            Expr::Dict(dict) => self.visit_dict_info(dict, token),
            Expr::Record(record) => self.visit_record_info(record, token),
            Expr::Set(set) => self.visit_set_info(set, token),
            Expr::Tuple(tuple) => self.visit_tuple_info(tuple, token),
            Expr::TypeAsc(type_asc) => self.visit_expr_info(&type_asc.expr, token),
            Expr::Dummy(dummy) => self.visit_dummy_info(dummy, token),
            Expr::Compound(block) | Expr::Code(block) => self.visit_block_info(block, token),
            Expr::ReDef(redef) => self.visit_redef_info(redef, token),
            Expr::Import(_) => None,
        }
    }

    fn visit_acc_info(&self, acc: &Accessor, token: &Token) -> Option<VarInfo> {
        match acc {
            ...
            Accessor::Attr(attr) => self
                ...
                .or_else(|| self.visit_expr_info(&attr.obj, token)),
        }
    }

    fn visit_bin_info(&self, bin: &BinOp, token: &Token) -> Option<VarInfo> {
        self.visit_expr_info(&bin.lhs, token)
            .or_else(|| self.visit_expr_info(&bin.rhs, token))
    }

    fn visit_call_info(&self, call: &Call, token: &Token) -> Option<VarInfo> {
        if let Some(attr) = &call.attr_name {
            if let Some(t) = self.return_var_info_if_same(attr, attr.name.token(), token) {
                return Some(t);
            }
        }
        self.visit_expr_info(&call.obj, token)
            .or_else(|| self.visit_args_info(&call.args, token))
    }

    fn visit_args_info(&self, args: &Args, token: &Token) -> Option<VarInfo> {
        for arg in args.pos_args.iter() {
            if let Some(expr) = self.visit_expr_info(&arg.expr, token) {
                return Some(expr);
            }
        }
        if let Some(var) = &args.var_args {
            if let Some(expr) = self.visit_expr_info(&var.expr, token) {
                return Some(expr);
            }
        }
        for arg in args.kw_args.iter() {
    ...
    fn visit_def_info(&self, def: &Def, token: &Token) -> Option<VarInfo> {
        self.return_var_info_if_same(def.sig.ident(), def.sig.ident().name.token(), token)
            .or_else(|| self.visit_block_info(&def.body.block, token))
    }

    fn visit_class_def_info(&self, class_def: &ClassDef, token: &Token) -> Option<VarInfo> {
        class_def
            .require_or_sup
            .as_ref()
            .and_then(|req_sup| self.visit_expr_info(req_sup, token))
            .or_else(|| {
            ...
            .or_else(|| self.visit_block_info(&class_def.methods, token))
    }

    fn visit_block_info(&self, block: &Block, token: &Token) -> Option<VarInfo> {
        for chunk in block.iter() {
            if let Some(expr) = self.visit_expr_info(chunk, token) {
                return Some(expr);
            }
    ...
    fn visit_redef_info(&self, redef: &ReDef, token: &Token) -> Option<VarInfo> {
        self.visit_acc_info(&redef.attr, token)
            .or_else(|| self.visit_block_info(&redef.block, token))
    }

    fn visit_dummy_info(&self, dummy: &Dummy, token: &Token) -> Option<VarInfo> {
        for chunk in dummy.iter() {
            if let Some(expr) = self.visit_expr_info(chunk, token) {
                return Some(expr);
            }
    ...
    fn visit_patchdef_info(&self, patch_def: &PatchDef, token: &Token) -> Option<VarInfo> {
        self.visit_expr_info(&patch_def.base, token)
            .or_else(|| self.visit_block_info(&patch_def.methods, token))
    }

    fn visit_lambda_info(&self, lambda: &Lambda, token: &Token) -> Option<VarInfo> {
        self.visit_block_info(&lambda.body, token)
    }

    fn visit_array_info(&self, arr: &Array, token: &Token) -> Option<VarInfo> {
        match arr {
            Array::Normal(arr) => self.visit_args_info(&arr.elems, token),
            _ => None, // todo!(),
        }
    }

    fn visit_dict_info(&self, dict: &Dict, token: &Token) -> Option<VarInfo> {
        match dict {
            Dict::Normal(dict) => {
                for kv in &dict.kvs {
                    if let Some(expr) = self.visit_expr_info(&kv.key, token) {
                        return Some(expr);
                    } else if let Some(expr) = self.visit_expr_info(&kv.value, token) {
                        return Some(expr);
                    }
                }
                None
    ...
    fn visit_record_info(&self, record: &Record, token: &Token) -> Option<VarInfo> {
        for field in record.attrs.iter() {
            if let Some(expr) = self.visit_block_info(&field.body.block, token) {
                return Some(expr);
            }
        }
        None
    ...
    fn visit_set_info(&self, set: &Set, token: &Token) -> Option<VarInfo> {
        match set {
            Set::Normal(set) => self.visit_args_info(&set.elems, token),
            _ => None, // todo!(),
        }
    }

    fn visit_tuple_info(&self, tuple: &Tuple, token: &Token) -> Option<VarInfo> {
        match tuple {
            Tuple::Normal(tuple) => self.visit_args_info(&tuple.elems, token),
            // _ => None, // todo!(),
        }
    }
}
