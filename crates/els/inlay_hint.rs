...
use erg_compiler::hir::{Call, ClassDef, Def, Expr, Signature};
...
fn type_anot<D: std::fmt::Display>(ln_end: u32, col_end: u32, ty: D) -> InlayHint {
    let position = Position::new(ln_end - 1, col_end);
    let label = InlayHintLabel::String(format!(": {ty}"));
...
impl<Checker: BuildRunnable> Server<Checker> {
    ...
    fn get_call_hint(&self, call: &Call) -> Vec<InlayHint> {
        ...
        for (i, pos_arg) in call.args.pos_args.iter().enumerate() {
            ...
            if let Some(name) = nd_ts.get(i).and_then(|pt| pt.name()) {
                let hint = param_anot(
                    pos_arg.ln_begin().unwrap(),
                    pos_arg.col_begin().unwrap(),
                    name,
                );
                result.push(hint);
            }
